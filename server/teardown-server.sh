...
if command -v docker &>/dev/null; then
    if [ "$ASSUME_YES" -ne 1 ]; then
        read -r -p "Arreter/supprimer le serveur d'expo (dockhand, dnsmasq, dnsmasq-admin, caddy, caddy-admin, webui, bastion, dashboard, git-mirror, bastion-ansible) ? [y/N] " ans
